...
//   required     → 필요 인원 (투입 가능 인원과 비교하는 기준, 라벨/색상용)
//   maxBlock     → 그 시간에 블락 가능한 최대 인원 (초과 시 "마감")
//   label        → 필요 열에 표시할 텍스트
//
...
export function requiredFor(slot) {
  ...
  if (m >= 9 * 60 && m < 11 * 60)                 // 09:00 ~ 11:00
    return { locked: true, required: 5, label: "5", maxBlock: 0 };
  if (m >= 11 * 60 && m < 12 * 60 + 30)           // 11:00 ~ 12:30
    return { locked: false, required: 2, label: "2", maxBlock: 3 };
  if (m >= 12 * 60 + 30 && m < 13 * 60 + 30)      // 12:30 ~ 13:30
    return { locked: true, required: 0, label: "점심", maxBlock: 0 };
  if (m >= 13 * 60 + 30 && m < 14 * 60)           // 13:30 ~ 14:00
    return { locked: false, required: 3, label: "3", maxBlock: 2 };
  if (m >= 14 * 60 && m <= 17 * 60)               // 14:00 ~ 17:00
    return { locked: false, required: 2, label: "2", maxBlock: 3 };
  return { locked: false, required: 0, label: "", maxBlock: 0 };
}
